...
async function saveNumbersToCsv(usuarios) {
    const stream = fs.createWriteStream('usuarios.csv');
  
    const filteredUsuarios = usuarios.filter(usuario => usuario.id_usuario && usuario.id_usuario.includes('@c.us'));
    const uniqueUsuarios = [...new Set(filteredUsuarios.map(usuario => usuario.id_usuario))];
    const usuariosToSave = filteredUsuarios.filter(usuario => uniqueUsuarios.includes(usuario.id_usuario));
  
    csv.write(
      usuariosToSave.map(usuario => ({
        "Given Name": usuario.nome,
        "Family Name": `cmds: ${usuario.comandos_total}`,
        "Phone 1 - Value": usuario.id_usuario && usuario.id_usuario.replace("@c.us", "")
      })),
      { headers: true }
    ).pipe(stream);
  }
    ...
    try{
        ...
        switch(command){
            ...
            case '!limpar':
              ...
              //          "*Grupo Oficial:*\n https://chat.whatsapp.com/DwHiCkxpribAvBU3752Tll",
              //          "*Grupo de Info:*\n https://chat.whatsapp.com/DwHiCkxpribAvBU3752Tll",
              //          // Add more group messages here but keep the same link
              //        ];
              ...
              //      //           await client.sendLinkWithAutoPreview(contactId, grupo);
